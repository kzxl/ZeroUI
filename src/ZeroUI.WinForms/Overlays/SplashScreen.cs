    ...
    public static class SplashScreen
    {
        ...
        private class SplashForm : Form
        {
            ...
            protected override void OnPaint(PaintEventArgs e)
            {
                ...
                int padX = (int)Math.Round(36 * _dpiScale);
                int padY = (int)Math.Round(44 * _dpiScale);
                ...
                // 3. Logo / Badge / Icon
                int logoSize = (int)Math.Round(48 * _dpiScale);
                ...
                {
                    // Render Primary Badge with Glyph
                    using (var logoPath = ZeroUIConfig.CreateRoundedRectangle(logoRect, (int)Math.Round(10 * _dpiScale)))
                    {
                        using var brush = new SolidBrush(palette.Primary);
                        g.FillPath(brush, logoPath);
                        using var glyphFont = new Font("Segoe UI", 20f * _dpiScale, FontStyle.Bold);
                ...
                int textLeft = logoRect.Right + (int)Math.Round(16 * _dpiScale);
                int textWidth = Width - textLeft - padX;

                using (var titleFont = new Font("Segoe UI", 16f * _dpiScale, FontStyle.Bold))
                {
                    Rectangle titleRect = new Rectangle(textLeft, padY, textWidth, (int)Math.Round(30 * _dpiScale));
                ...
                using (var subFont = new Font("Segoe UI", 9.5f * _dpiScale, FontStyle.Regular))
                {
                    Rectangle subRect = new Rectangle(textLeft, padY + (int)Math.Round(30 * _dpiScale), textWidth, (int)Math.Round(20 * _dpiScale));
                ...
                // 5. Progress Bar
                int barY = Height - (int)Math.Round(105 * _dpiScale);
                int barH = (int)Math.Round(6 * _dpiScale);
                Rectangle barRect = new Rectangle(padX, barY, Width - (padX * 2), barH);

                if (_options.ShowProgressBar)
                {
                    using (var barPath = ZeroUIConfig.CreateRoundedRectangle(barRect, (int)Math.Round(3 * _dpiScale)))
                    {
                        ...
                        if (_progress.HasValue)
                        {
                            ...
                            if (fillW > 0)
                            {
                                ...
                                using var fillPath = ZeroUIConfig.CreateRoundedRectangle(fillRect, (int)Math.Round(3 * _dpiScale));
                        ...
                        else
                        {
                            ...
                            {
                                using var shimmerPath = ZeroUIConfig.CreateRoundedRectangle(shimmerRect, (int)Math.Round(3 * _dpiScale));
                ...
                // 6. Status Text & Percentage
                int statusY = barY + barH + (int)Math.Round(10 * _dpiScale);
                using (var statusFont = new Font("Segoe UI", 9f * _dpiScale, FontStyle.Regular))
                {
                    ...
                    Rectangle statusRect = new Rectangle(padX, statusY, statusWidth, (int)Math.Round(22 * _dpiScale));
                    ...
                    {
                        Rectangle pctRect = new Rectangle(Width - padX - pctWidth, statusY, pctWidth, (int)Math.Round(22 * _dpiScale));
                ...
                // 7. Footer Version / Copyright & Optional Cancel Button
                int footerY = Height - (int)Math.Round(36 * _dpiScale);
                if (_options.ShowFooter && !string.IsNullOrEmpty(_options.FooterText))
                {
                    using var footFont = new Font("Segoe UI", 7.5f * _dpiScale, FontStyle.Regular);
                    int cancelAlloc = _options.EnableCancellation ? (int)Math.Round(80 * _dpiScale) : 0;
                    Rectangle footRect = new Rectangle(padX, footerY, Width - (padX * 2) - cancelAlloc, (int)Math.Round(20 * _dpiScale));
                ...
                // 8. Cancellation Button (if enabled)
                if (_options.EnableCancellation)
                {
                    int btnW = (int)Math.Round(68 * _dpiScale);
