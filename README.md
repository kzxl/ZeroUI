...
| **`ZeroUI.Wpf`** | `net462`, `net8.0-windows` | Zero-alloc WPF virtual grid (`GridControl`), industrial styling, 15+ specialized charts, and D3D11 shared texture bridge | `ZeroUI.Core` |
| **`ZeroUI.Wpf.Media`** | `net462`, `net8.0-windows` | High-performance interactive imaging & creative media studio suite (`ZImageViewer`, `ZPictureEdit`, `ZCompareViewer`, `ZCropBox`, `ZHistogramScope`, `ZCurveEditor`, `ZColorWheel`, `ZThumbnailGrid`, `ZDominantPalette`, `ZHistoryTimeline`, `ZFilmstripScroller`, `ZMaskGizmoOverlay`, `ZBatchTaskQueue`, `ZTokenPatternEditor`, `ZExifTelemetryCard`, `ZMiniMapNavigator`) | `ZeroUI.Core`, `ZeroGraphics` |
